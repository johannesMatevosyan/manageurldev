...
config. BalloonImgPath = null // Path to images (border, corners, stem), in quotes. Path must be relative to your HTML file.
...
function tt_GetClientH()
{
	...
	return(   tt_db ? (tt_db.clientHeight || 0)
			: document.body && (typeof(document.body.clientHeight) != tt_u) ? document.body.clientHeight
...
function tt_Init()
{
	...
	tt_Hide();
	tt_scrlX = tt_GetScrollX(); // init X scroll
	tt_scrlY = tt_GetScrollY(); // init Y scroll
	config.BalloonImgPath = _imagePath + "/tip_balloon/";
...
function tt_Tip(arg, t2t)
{
	if(!tt_db)
		return;
	// patch to show tooltip without delay if alredy exist one
	var nodelay = false;
	if(tt_iState) {
		nodelay = true;
		tt_Hide();
...
function tt_ShowInit(nodelay)
{
	if (nodelay)
		tt_tShow.Timer("tt_Show()", 0, true);
	else
